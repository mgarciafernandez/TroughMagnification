		...
		for qt_ in __quantile__:
			...
			for band_ in ['mag_auto_i']:
				for ii_ in xrange(5):
					...
					plt.savefig('./control_plots_troughs/phiz_'+band_+str(ii_)+'.png')

					plt.cla()
					plt.hist2d(mySour['ra'],mySour['dec'],bins=[100,100])
					plt.savefig('./control_plots_troughs/density_sour_'+band_+str(ii_)+'.png')

					plt.cla()
					plt.hist2d(lens['ra'],lens['dec'],bins=[100,100])
					plt.savefig('./control_plots_troughs/density_lens_'+band_+str(ii_)+'.png')

					lens_cat = treecorr.Catalog(ra=lens['ra'],dec=lens['dec'],w=lens['depth'],ra_units='degrees',dec_units='degrees')
					...
					dd.write('./results_number_counts_troughs/w_nc_troughs_ap'+str(ap_)+'_qt'+str(qt_)+'_'+band_+str(ii_),dr=dr,rd=rd,rr=rr)
