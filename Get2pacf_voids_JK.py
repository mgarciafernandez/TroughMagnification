...
if __name__ == "__main__":
	...
	for band_ in ['mag_auto_i']:
		for ii_ in xrange(5):
			...
			for v_ in xrange(len(lens)):
				
				myLens = numpy.hstack([ lens[:v_],lens[v_+1:] ])

				lens_cat = treecorr.Catalog(ra=myLens['ra'],dec=myLens['dec'],ra_units='degrees',dec_units='degrees')
				...
				dd.write('./results_number_counts_voids/w_nc_'+band_+str(ii_)+'_jk'+str(v_),dr=dr,rd=rd,rr=rr)
