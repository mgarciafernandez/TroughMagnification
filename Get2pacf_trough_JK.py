...
if __name__ == "__main__":
	...
	for ap_ in __apperture__:
		for qt_ in __quantile__:
			lens = fits.open('./processed_data/y1a1_trough_centers_'+ap_+'_'+qt_+'.fits')[1].data
			ranl = fits.open('./processed_data/trough_'+ap_+'_random_uniform_masked.fits')[1].data
	
			lens = lens[ lens['mask'] == 1]
			ranl = ranl[ ranl['mask'] == 1]
			
			lens = lens[ (-60 < lens['dec']) & (lens['dec'] < -40) ]
			ranl = ranl[ (-60 < ranl['dec']) & (ranl['dec'] < -40) ]

			KM = KMeans(__Njk__,n_init=1,init='random',n_jobs=15)

			ranl_jk = numpy.array(zip(numpy.sin(ranl['dec']*numpy.pi/180.)*numpy.cos(ranl['ra']*numpy.pi/180.),\
                                                  numpy.sin(ranl['dec']*numpy.pi/180.)*numpy.sin(ranl['ra']*numpy.pi/180.) ) )
			lens_jk = numpy.array(zip(numpy.sin(lens['dec']*numpy.pi/180.)*numpy.cos(lens['ra']*numpy.pi/180.),\
                                                  numpy.sin(lens['dec']*numpy.pi/180.)*numpy.sin(lens['ra']*numpy.pi/180.) ) )

			KM.fit( ranl_jk )
			ranl_index = KM.predict( ranl_jk )
			lens_index = KM.predict( lens_jk )

			for band_ in ['mag_auto_i']:
				for ii_ in xrange(5):

					print band_,ii_,ap_,qt_
	
					mySour = sour[ sour[band_] < __edges__[band_][ii_] ]

					for jk_ in xrange(__Njk__):
						lens_cat = treecorr.Catalog(ra=lens[lens_index != jk_]['ra'],dec=lens[lens_index != jk_]['dec'],w=lens[lens_index != jk_]['depth'],ra_units='degrees',dec_units='degrees')
						ranl_cat = treecorr.Catalog(ra=ranl[ranl_index != jk_]['ra'],dec=ranl[ranl_index != jk_]['dec'],ra_units='degrees',dec_units='degrees')
						...
						dd = treecorr.NNCorrelation(nbins=7,min_sep=0.1,max_sep=20,sep_units='degrees')
						dr = treecorr.NNCorrelation(nbins=7,min_sep=0.1,max_sep=20,sep_units='degrees')
						rd = treecorr.NNCorrelation(nbins=7,min_sep=0.1,max_sep=20,sep_units='degrees')
						rr = treecorr.NNCorrelation(nbins=7,min_sep=0.1,max_sep=20,sep_units='degrees')
						...
						dd.write('./results_number_counts_troughs/w_nc_troughs_ap'+str(ap_)+'_qt'+str(qt_)+'_'+band_+str(ii_)+'_jk'+str(jk_),dr=dr,rd=rd,rr=rr)
